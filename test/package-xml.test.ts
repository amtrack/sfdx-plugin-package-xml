...
describe("PackageXml", () => {
  const destructiveChanges = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "fixtures", "package", "destructiveChanges.json"),
      "utf8"
    )
  );
  const managedPackage = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "fixtures", "package", "managedPackage.json"),
      "utf8"
    )
  );
  const simplePackage = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "fixtures", "package", "package.json"),
      "utf8"
    )
  );
