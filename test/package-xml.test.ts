...
import { PackageXml, compareAlphanumeric, comparePackageXmlProperties } from "../src/package-xml";

describe("PackageXml", () => {
  const destructiveChanges = JSON.parse(
    fs.readFileSync(path.join(__dirname, "fixtures", "package", "destructiveChanges.json"), "utf8"),
  );
  const managedPackage = JSON.parse(
    fs.readFileSync(path.join(__dirname, "fixtures", "package", "managedPackage.json"), "utf8"),
  );
  ...
  describe("#toString()", () => {
    ...
    it("should return the xml representation of a managed package", () => {
      const expectedXml = fs
        .readFileSync(path.resolve(__dirname, "fixtures", "package", "managedPackage.xml"))
        .toString();
    ...
    it("should return the xml representation of a destructive changes package", () => {
      const expectedXml = fs
        .readFileSync(path.resolve(__dirname, "fixtures", "package", "destructiveChanges.xml"))
        .toString();
