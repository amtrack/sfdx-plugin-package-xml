...
export class RegularMetadata extends MetadataLister {
  ...
  public async run(
    ...
    const metadataQueries = describeMetadataResult.metadataObjects.map((cmp) => {
      return {
        type: cmp.xmlName,
      };
    });
    const filteredMetadataQueries = this.filterTypes(metadataQueries, (x) => `${x.type}`);
...
export function fixCustomFeedFilter(fileProperties: FileProperties[]): FileProperties[] {
  return fileProperties.map((fileProperty) => {
    if (fileProperty.type === "CustomFeedFilter") {
      ...
      if (!fileProperty.fullName.includes(".")) {
...
export function ignoreFlowDefinition(fileProperties: FileProperties[], apiVersion: string): FileProperties[] {
  if (apiVersion >= "44.0") {
    return fileProperties.filter((fileProperty) => fileProperty.type !== "FlowDefinition");
  }
